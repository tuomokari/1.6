using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;
using SystemsGarden.mc2.MC2SiteEnvironment;
using SystemsGarden.mc2.Core.Runtime;
using SystemsGarden.mc2.Common;
using MongoDB.Driver.Builders;
using MongoDB.Driver;
using MongoDB.Bson;
using SystemsGarden.mc2.Core.Runtime;
using SystemsGarden.mc2.Common;
using MongoDB.Driver.Builders;
using MongoDB.Driver;
using MongoDB.Bson;
using SystemsGarden.mc2.Core.Runtime;

namespace SystemsGarden.mc2.widgets.routefieldwidget
{
    public class routefieldwidget : MC2Controller
    {
        #region Actions

        /// <summary>
        /// Generated code for MC2 widget "routefieldwidget". Can be removed or replaced
        /// with actual implementation. Please remember to change or remove the GrantAccessToGroup
        /// property.
        /// </summary>
        [GrantAccessToGroup("authenticated")]
        public ActionResult addnewlocation(string name)
        {
            var location = new DBDocument("location");

            location["name"] = name;

			location.UpdateDatabase();

            return new AjaxResult((string)location["_id"]);
        }

        #endregion
    }
}