...
$.widget("workdatawidget.workdatawidget", $.core.base, {
    ...
    _getPersistedValues: function _getPersistedValues() {
        ...
        if (!this._model.selectedDateKey) this._model.selectedDateKey = this.getKeyFromDate(new Date());
        ...
        if (this.options.lastnewid && this.options.lastnewid != sessionStorage.getItem(this._model.currentUserId + "_lastnewid") && this.options.lastnewcollectionname) {
     ...
     */
    _setupWidgetEvents: function _setupWidgetEvents() {
        var base = this;
    },
    /**
    ...
    *
    ...
    refreshData: function refreshData(options) {
        ...
        baseDataPromise.done(function (data) {
            ...
            if (data.timesheetentry) base._model.timesheetEntries = data.timesheetentry;

            if (data.absenceentry) base._model.absenceEntries = data.absenceentry;

            if (data.dayentry) base._model.dayEntries = data.dayentry;

            if (data.articleentry) base._model.articleEntries = data.articleentry;

            if (data.allocationentry) {
                base._model.allocationEntries = data.allocationentry;
                base._model.allocationEntries.sort(base._endtimeSortFunction);
            }
            if (data.assetentry) base._model.assetEntries = data.assetentry;
        ...
        payTypesPromise.done(function (data) {
            if (!data.timesheetentrydetailpaytype) throw "No paytypes found";
    ...
    _addEntriesFromArrayToObject: function _addEntriesFromArrayToObject(documentArray, documentObject) {
    ...
    */
    _determineNextAndPreviousDay: function _determineNextAndPreviousDay() {
    ...
    _cacheDataById: function _cacheDataById() {
        // Only cache what's needed. More types can be added here.
        this._cacheCollectionById("allocationEntries");
    },
    _cacheCollectionById: function _cacheCollectionById(collectionName) {
    ...
    setCurrentDateAsCopied: function setCurrentDateAsCopied() {
    ...
    setSelectedDateKey: function setSelectedDateKey(dateKey) {
    ...
    scrollHorizontalWorkView: function scrollHorizontalWorkView() {
        $(this.element).trigger("scrollrequest");
    },
    /*
    * Get string representig date part of timestamp
    */
    getKeyFromDate: function getKeyFromDate(date) {
    ...
    getDateFromKey: function getDateFromKey(key) {
     ...
     */
    startProject: function startProject(allocationId) {
     ...
     */
    endProject: function endProject(allocationId) {
     ...
     */
    _convertWorkDataDates: function _convertWorkDataDates() {
        var base = this;
     ...
     * Converty ISO date strings to JS dates.
     */
    _convertDates: function _convertDates(dateEntryArray) {
        for (var i = 0; i < dateEntryArray.length; i++) {
            var entry = dateEntryArray[i];

            if (entry.date) entry.date = getDateFromIsoString(entry.date);

            if (entry.starttimestamp) entry.starttimestamp = getDateFromIsoString(entry.starttimestamp);

            if (entry.endtimestamp) entry.endtimestamp = getDateFromIsoString(entry.endtimestamp);

            if (entry.created) entry.created = getDateFromIsoString(entry.created);
        }
    },
    /*
     ...
     */
    _generateDaysForRange: function _generateDaysForRange() {
        var base = this;
        ...
        while (current.diff(end) <= 0) {
            ...
            if (day.key === todaysKey) day.isToday = true;

            base._model.days[day.key] = day;
        }
    },
    _splitWorkDataToDays: function _splitWorkDataToDays() {
     ...
     */
    _splitDataWithDatesToDays: function _splitDataWithDatesToDays(entries, type) {
    ...
    _addTimedEntries: function _addTimedEntries() {
    ...
    _splitAssetDataForAssets: function _splitAssetDataForAssets() {
        var days = this._getDaysArray();
    ...
    _sortEntries: function _sortEntries() {
        var days = this._getDaysArray();
    ...
    _findEntryToHighlight: function _findEntryToHighlight() {
    ...
    _timestampSortFunction: function _timestampSortFunction(a, b) {
    ...
    _endtimeSortFunction: function _endtimeSortFunction(a, b) {
        if (a.endtimestamp) {
            if (b.endtimestamp) {
        ...
        if (!a.project.__displayname && !b.project.__displayname) return 0;

        if (!a.project.__displayname) return -1;

        if (!b.project.__displayname) return 1;

        if (a.project.__displayname == b.project.__displayname) return 0;

        return a.project.__displayname < b.project.__displayname ? -1 : 1;
    },
    _sortTimedEntries: function _sortTimedEntries(day) {
        day.timedEntries.sort(this._timestampSortFunction);
    },
    _sortAllocations: function _sortAllocations(day) {
        day.allocationEntries.sort(this._timestampSortFunction);
    },
    /**
     * Count total values per day.
     */
    _countDailyTotals: function _countDailyTotals() {
        ...
        for (var i = 0; i < days.length; i++) {
            ...
            for (var j = 0; j < day.timesheetEntries.length; j++) {
                ...
                if (payType.isovertime50) day.overtime50Hours += durationHours;else if (payType.isovertime100) day.overtime100Hours += durationHours;else if (payType.isovertime150) day.overtime150Hours += durationHours;else if (payType.isovertime200) day.overtime200Hours += durationHours;
                ...
                if (!timesheetEntry.approvedbyworker) {
                    ...
                    if (timesheetEntry.creator._id == this._model.currentUserId) day.hasUnapprovedCurrentUserCreatedEntries = true;
                } else {
            ...
            for (var j = 0; j < day.absenceEntries.length; j++) {
                ...
                if (!absenceEntry.approvedbyworker) {
                    day.hasUnapprovedEntries = true;
                    if (absenceEntry.creator._id == this._model.currentUserId) day.hasUnapprovedCurrentUserCreatedEntries = true;
                } else {
                    day.hasApprovedEntries = true;
                }
            }
            ...
            for (var j = 0; j < day.articleEntries.length; j++) {
                ...
                if (!articleEntry.approvedbyworker) {
                    day.hasUnapprovedEntries = true;
                    if (articleEntry.creator._id == this._model.currentUserId) day.hasUnapprovedCurrentUserCreatedEntries = true;
                } else {
                    day.hasApprovedEntries = true;
                }
            }
            ...
            for (var j = 0; j < day.assets.length; j++) {
                ...
                for (var k = 0; k < asset.hours.length; k++) {
                    ...
                    if (!assetEntry.approvedbyworker) {
                        day.hasUnapprovedEntries = true;
                        if (assetEntry.creator._id == this._model.currentUserId) day.hasUnapprovedCurrentUserCreatedEntries = true;
                    } else {
                        day.hasApprovedEntries = true;
                    }
                }
            ...
            for (var j = 0; j < day.dayEntries.length; j++) {
                ...
                if (!dayEntry.approvedbyworker) {
                    day.hasUnapprovedEntries = true;
                    if (dayEntry.creator._id == this._model.currentUserId) day.hasUnapprovedCurrentUserCreatedEntries = true;
                } else {
                    day.hasApprovedEntries = true;
                }
     ...
     */
    _splitEntriesForProjects: function _splitEntriesForProjects(day) {
        var days = this._getDaysArray();
    ...
    _verifyProjectExists: function _verifyProjectExists(day, id) {
    ...
    */
     ...
     */
    _getDaysArray: function _getDaysArray() {
        var base = this;
        var current = moment(base._model.startDate);
        var end = moment(base._model.endDate);
    ...
    _millisecondsToHours: function _millisecondsToHours(milliseconds) {
